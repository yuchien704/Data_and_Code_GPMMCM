...
EEI.GMIXCMB.ECM.new = function(Ycm, Y.knn, cen, g, true.clus, tol = 1e-5, neq.S = T, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
...
if(g==1) class=rep(1,n) else class=kmeans(Y.knn, g)$cluster
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'EEI.GMIXCMB: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
