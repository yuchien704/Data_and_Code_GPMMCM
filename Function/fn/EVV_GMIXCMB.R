...
EVV.GMIXCMB.ECM.new = function(Ycm, Y.knn, cen, g, true.clus, tol = 1e-5, neq.S = T, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'EVV.GMIXCMB: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
