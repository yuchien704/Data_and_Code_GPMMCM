...
EVE.GMIXCMB.ECM.new = function(Ycm, Y.knn, cen, g, true.clus, tol = 1e-5, neq.S = T, max.iter, tol.FG, max.iter.FG, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'EVE.GMIXCMB: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    for(i in 1:g)
    {
      Aj[,,i] = diag(diag(t(D)%*%W[,,i]%*%D))/det(diag(diag(t(D)%*%W[,,i]%*%D)))^(1/p) 
      W.l[i] = sum(diag(D%*%solve(Aj[,,i])%*%t(D)%*%W[,,i]))    
    }  
    lambda=sum(W.l)/(n*p) 
    # new log-likelihood
    for (i in 1:g) {
