        ...
        if(pjf[j] == pjo[j]){
          ...
          if(pjf[j] == 0){
            ...
            if(sum(cen[j,]!=0,na.rm = T) == 1){
              ...
              log.cdf[j,i] = log(abs(pmvnorm(lower=A, upper=B, mean=c(mu.cf), sigma=Sig.cc.f)))[1]
            }
          }else{
...
VVV.GMIXCMB.ECM.new = function(Ycm, Y.knn, cen, g, true.clus, tol = 1e-5, neq.S = T, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'VVV.GMIXCMB: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    for(i in 1:g)
    {
      ...
      Sigma[,,i] = (apply(rep(Z[,i], each=p^2)*y2.hat[,,,i], 1:2, sum) - colSums(Z[,i]*y.hat[,,i])%*%t(mu[,i]) - 
                      mu[,i]%*%t(colSums(Z[,i]*y.hat[,,i])) + sum(Z[,i])*mu[,i]%*%t(mu[,i])) / ni[i]  #更新Sigma值
    } 
    # new log-likelihood
    w.den = log.like.B(Ycm, cen, na, w, mu, Sigma, Oj, Cj, Fj)
    max.wden = apply(w.den, 1, max)
  ...
  alpha=g*p+g-1;beta=g*(p*(p+1)/2);m=alpha+beta  #總參數個數
