...
VEV.GMIXCMB.ECM.new = function(Ycm, Y.knn, cen, g, true.clus, tol = 1e-5, neq.S = T, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  omega.lamb = array(NA, dim = c(p,p,g))
  if(g==1) class=rep(1,n) else class=kmeans(Y.knn, g)$cluster
   w = table(class)/nrow(Y.knn)
   for(i in 1:g)
    {
    mu[,i] = colMeans(Y.knn[class == i,]) #把第i 群的Y 資料取出算第i 群平均
    Sigma[,,i] = cov(Y.knn[class == i,])  #把第i 群的Y 資料取出算第i 群變異數矩陣
    lambda[i]=(det(Sigma[,,i]))^(1/p)
    }
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'VEV.GMIXCMB: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    for(i in 1:g)
    {
      ...
      L[,,i] = eigen(W[,,i])$vectors
      omega[,,i] = diag(eigen(W[,,i])$values)
      omega.lamb[,,i] = omega[,,i]/lambda[i]
    } 
    Aj = apply(omega.lamb, c(1,2), sum)/ det(apply(omega.lamb, c(1,2), sum))^(1/p) #Ahat 
    for (i in 1:g) {
      lambda[i]=sum(diag(W[,,i]%*%L[,,i]%*%solve(Aj)%*%t(L[,,i]))) / (p*ni[i])
      Sigma[,,i]=lambda[i] * (L[,,i] %*% Aj %*% t(L[,,i]))
    }
 ...
 if(length(unique(post.clus))==length(unique(true.clus)))
  ...
  alpha=g*p+g-1;beta=p*(p+1)/2;m=alpha+g*beta-(g-1)*(p-1)  #總參數個數
