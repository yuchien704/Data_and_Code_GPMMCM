...
EVI.GMIX.MVNCM.B.ECM = function(Ycm, cen, g, clus, tol = 1e-5, neq.S = T, max.iter, tol.FG=1e-5, max.iter.FG = 100, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'EVI.GMIX.MVNCM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    for(i in 1:g)
    {
      ...
      Bj[,,i]= diag(diag(sum.Ome[,,i]))/det(diag(diag(sum.Ome[,,i])))^(1/p)
      W.l[i]=det(diag(diag(sum.Ome[,,i])))^(1/p)
    } 
    lambda=sum(W.l)/n
  ...
  #=========================#
  alpha=g*p+g-1;m=alpha+g*p-g+1       #總參數個數
