...
EVE.GMIX.MVNCM.B.ECM_MM = function(Ycm, cen, g, clus, tol = 1e-5, neq.S = T, max.iter, tol.MM, max.iter.MM, per)
{
  begin = proc.time()[1]
  iter = 0
  # Basic setting
  ...
  for(i in 1:g)
  {
    ...
    Sigma[,,i] =  var(na.omit(Ycm))
    #Aj0 = diag(rhalfnorm(p, theta=sqrt(pi/2)))
    #Aj[,,i] = Aj0/(det(Aj0)^(1/p))
    #cent = t(Ycm) - mu[,i]
  }
  #if(neq.S != T)
  #{
  #  Sigma = apply(Sigma, 2, rowSums) / g
  #  Sigma = array(rep(Sigma, g), dim = c(p,p,g))
  #}
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'EVE.GMIX.MVNCM_MM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    for(i in 1:g)
    {
      ...
      #Sigma[,,i] = sum.Ome[,,i] / ni[i] #更新Sigma值
      #Aj[,,i] = diag(eigen(Sigma[,,i])$val)/(det(diag(eigen(Sigma[,,i])$val))^(1/p))
    } 
    S = sum.Ome
    #D = MM_algorithm(p, g, S, A=Aj, tol.MM, max.iter.MM)$D
    #D = MM_algorithm(p, g, S, Sigma, tol.MM, max.iter.MM)$D
    D = MM1(p, g, S, Sigma, tol.MM, max.iter.MM)$D
    for(i in 1:g)
    {
      Aj[,,i] = diag(diag(t(D)%*%sum.Ome[,,i]%*%D))/det(diag(diag(t(D)%*%sum.Ome[,,i]%*%D)))^(1/p) 
      W.l[i] = sum(diag(D%*%solve(Aj[,,i])%*%t(D)%*%sum.Ome[,,i]))    
    }  
    lambda=sum(W.l)/(n*p) 
    # new log-likelihood
    for (i in 1:g) {
      Sigma[,,i]=lambda * (D %*% Aj[,,i] %*% t(D)) 
    }
    # new log-likelihood
    w.den = log.like_B(Ycm, cen, cen1, w, mu, Sigma, yc, Oj, Cj, Fj, cen.subj, cen.posi)
  ...
  #=========================#
  alpha=g*p+g-1;beta=p*(p+1)/2;m=alpha+beta+(g-1)*(p-1)      #總參數個數
...
#g=5; clus = kmeans(Y, g)$cluster; max.iter=1000;per=1
#Test_1 = EVE.GMIX.MVNCM.B.ECM_MM(Ycm, cen, g, clus, tol = 1e-5, neq.S = T, max.iter, tol.FG=1e-5, max.iter.FG = 100, per)
