...
EVV.GMIX.MVNCM.B.ECM.renew = function(Y, Ycm, cen, g, tol = 1e-5, neq.S = T, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'EVV.GMIX.MVNCM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    for(i in 1:g)
    {
      ...
      C[,,i]= sum.Ome[,,i]/det(sum.Ome[,,i])^(1/p) 
      W.l[i]=det(sum.Ome[,,i])^(1/p) 
    } 
    lambda=sum(W.l)/n 
  ...
  #=========================#
  alpha=g*p+g-1;beta=p*(p+1)/2;m=alpha+g*beta-(g-1)      #總參數個數
