...
EII.GMIX.MVNCM.B.ECM = function(Ycm, cen, g, clus, tol = 1e-5, neq.S = F, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'EII.GMIX.MVNCM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
