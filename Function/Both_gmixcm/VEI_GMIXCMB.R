...
VEI.GMIX.MVNCM.B.ECM = function(Ycm, cen, g, clus, tol = 1e-5, neq.S = T, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'VEI.GMIX.MVNCM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    W.l.sum=apply(W.l, c(1,2), sum)
    Bj= diag(diag(W.l.sum))/det(diag(diag(W.l.sum)))^(1/p)
    
    for (i in 1:g) {
      lambda[i]=sum(diag(solve(Bj)%*%sum.Ome[,,i]))/(p*ni[i])
      Sigma[,,i]=lambda[i]*Bj 
    }
  ...
  #=========================#
  alpha=g*p+g-1;m=alpha+p+g-1 #總參數個數
