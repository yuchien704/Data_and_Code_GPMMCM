...
VVE.GMIX.MVNCM.B.ECM_MM = function(Ycm, cen, g, clus, tol = 1e-5, neq.S = T, max.iter, tol.MM, max.iter.MM,per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'VVE.GMIX.MVNCM_MM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    #D = MM2(p, g, S, Sigma, tol.MM, max.iter.MM)$D

    for(i in 1:g)
    {
      Aj[,,i] = diag(diag(t(D)%*%sum.Ome[,,i]%*%D))/ni[i]
      Sigma[,,i]=D %*% Aj[,,i] %*% t(D)    
    }  
    ...
    diff.lnL = abs(new.loglik-old.loglik)
  ...
  #=========================#
  alpha=g*p+g-1;beta=p*(p+1)/2;m=alpha+beta+(g-1)*p     #總參數個數
