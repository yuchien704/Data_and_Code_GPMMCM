...
VEE.GMIX.MVNCM.B.ECM = function(Ycm, cen, g, clus, tol = 1e-5, neq.S = T, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'VEE.GMIX.MVNCM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    C= apply(W.l, c(1,2), sum)/ det(apply(W.l, c(1,2), sum))^(1/p)
    
    for (i in 1:g) {
      lambda[i]=sum(diag(solve(C)%*%sum.Ome[,,i]))/(p*ni[i])
      Sigma[,,i]=lambda[i]*C 
    }
  ...
  #=========================#
  alpha=g*p+g-1;beta=p*(p+1)/2;m=alpha+beta+g-1 #總參數個數
