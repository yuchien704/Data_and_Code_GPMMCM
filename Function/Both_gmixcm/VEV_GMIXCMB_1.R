...
VEV.GMIX.MVNCM.B.ECM.renew = function(Y, Ycm, cen, g, tol = 1e-5, neq.S = T, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'VEV.GMIX.MVNCM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    for(i in 1:g)
    {
      ...
      #W.l[,,i] = sum.Ome[,,i]/lambda[i]
      L[,,i]=eigen(sum.Ome[,,i])$vectors
      omega[,,i]=diag(eigen(sum.Ome[,,i])$values)
      omega.lamb[,,i]=omega[,,i]/lambda[i]
    }
    A= apply(omega.lamb, c(1,2), sum)/ det(apply(omega.lamb, c(1,2), sum))^(1/p) #Ahat
    
    for (i in 1:g) {
      lambda[i]=sum(diag(sum.Ome[,,i]%*%L[,,i]%*%solve(A)%*%t(L[,,i]))) / (p*ni[i])
      Sigma[,,i]=lambda[i] * (L[,,i] %*% A %*% t(L[,,i]))
      #cent = t(Y) - mu[,i]
    }
