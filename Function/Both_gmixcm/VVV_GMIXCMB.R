...
VVV.GMIX.MVNCM.B.ECM = function(Ycm, cen, g, clus, tol = 1e-5, neq.S = T, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  for(i in 1:g)
  {
    ...
    Sigma[,,i] = var(Ycm[clus == i,],na.rm = T)
  }
  if(neq.S != T)
  {
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'VVV.GMIX.MVNCM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
