...
EEI.GMIX.MVNCM.B.ECM.renew = function(Y, Ycm, cen, g, tol = 1e-5, neq.S = F, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  for(i in 1:g)
  {
    ...
    Sigma[,,i] = diag(diag(var(na.omit(Ycm))))
  }
  if(neq.S != T)
  {
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'EEI.GMIX.MVNCM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    for(i in 1:g)
    {
      ...
      { #遺失情況
        if(num.cen!=0){ #有遺失、有設限(M+C,M+C+F)
          ...
          ym2.hat = ym.hat %*% t(ym.hat) + Sig.mm.o
          ...
          #y2.hat = t(TM)%*%ym2.hat%*%TM + t(TM)%*%ym.hat%*%t(yf)%*%TF%*%TO + t(TM)%*%t(ycm.hat)%*%TC%*%TO + t(TO)%*%t(TF)%*%yf%*%t(ym.hat)%*%TM + t(TO)%*%t(TF)%*%yf%*%t(yf)%*%TF%*%TO + t(TO)%*%t(TF)%*%yf%*%t(yc.hat)%*%TC%*%TO + t(TO)%*%t(TC)%*%ycm.hat%*%TM + t(TO)%*%t(TC)%*%yc.hat%*%t(yf)%*%TF%*%TO + t(TO)%*%t(TC)%*%yc2.hat%*%TC%*%TO
        }else{ #有遺失、沒設限(M+F)
          TSig.oo.inv = matrix(0, no, no)
          for(j in 1:n){
            if(j == 1){
    ...
    W.sum=apply(sum.Ome, c(1,2), sum)
    Bj = diag(diag(W.sum))/det(diag(diag(W.sum)))^(1/p)
    lambda=det(diag(diag(W.sum)))^(1/p)/n
  ...
  #=========================#
  alpha=g*p+g-1;m=alpha+p   #總參數個數
  aic = 2 * m - 2 * new.loglik
  bic = m * log(n) - 2 * new.loglik
  model.inf = c(run.sec = run.sec, iter = iter,loglik = new.loglik, aic = aic, bic = bic, m=m)
