...
VVE.GMIX.MVNCM.B.ECM.renew = function(Y, Ycm, cen, g, tol = 1e-5, neq.S = T, max.iter, tol.FG=1e-5, max.iter.FG = 100, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'VVE.GMIX.MVNCM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
