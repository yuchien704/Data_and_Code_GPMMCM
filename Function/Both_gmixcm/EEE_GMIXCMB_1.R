...
EEE.GMIX.MVNCM.B.ECM.renew = function(Y, Ycm, cen, g, tol = 1e-5, neq.S = F, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'EEE.GMIX.MVNCM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    for(i in 1:g)
    {
      ...
      sum.Ome = Omega[1:p, 1:p]
      for(j in 2: n) sum.Ome = sum.Ome + Omega[(cumsum.p[j-1]+1):cumsum.p[j], (cumsum.p[j-1]+1):cumsum.p[j]]
      Sigma[,,i] = sum.Ome/ ni[i] #更新Sigma值
      eq.S = eq.S + w[i] * Sigma[,,i]
    } 
    Sigma = array(rep(eq.S, g), dim = c(p,p,g))
  ...
  #=========================#
  alpha=g*p+g-1;beta=(p*(p+1)/2);m=alpha+beta  #總參數個數
