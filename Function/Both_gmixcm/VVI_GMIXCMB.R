...
VVI.GMIX.MVNCM.B.ECM = function(Ycm, cen, g, clus, tol = 1e-5, neq.S = T, max.iter, per)
{
  begin = proc.time()[1]
  iter =0
  # Basic setting
  ...
  cat(paste(rep('-', 20), sep = '', collapse = ''), 'VVI.GMIX.MVNCM: ', 'g =', g, paste(rep('-', 20), sep = '', collapse = ''), '\n')
  ...
  repeat
  {
    ...
    for(i in 1:g)
    {
      ...
      lambda[i]=det(diag(diag(sum.Ome[,,i])))^(1/p)/ni[i]
